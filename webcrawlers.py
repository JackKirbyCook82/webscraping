...
_aslist = lambda items: [items] if hasattr(items, '__iter__') else [item for item in items]
...
class WebCrawler(object):
    ...
    def __call__(self, url, *args, **kwargs):
        for mainnum, pagenum, pageurl in self.execute(url, *args, **kwargs):
    ...
    def execute(self, mainurl, *args, crawlxpath, pagexpath, crawlnum=0, pagenum=0, datatype='html', **kwargs):
        ...
        nexturl = _aslist(mainhtmltree.xpath(crawlxpath))
        if len(nexturl) == 0: pass
        elif len(nexturl) == 1: yield from self.execute(nexturl, *args, crawlxpath=crawlxpath, pagexpath=pagexpath, crawlnum=crawlnum+1, pagenum=pagenum, **kwargs)
        else: raise ValueError(nexturl)
